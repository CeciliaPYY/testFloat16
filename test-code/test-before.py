...
class BKData1(ImageFolderDataset):
    def __init__(self, *arg1, **arg2):
        super(BKData1, self).__init__(*arg1,**arg2)
...
test_data1 = gluon.data.DataLoader(
    BKData1(test_path).transform_first(transform_test),
    batch_size=batch_size, shuffle=True, num_workers = num_workers,last_batch = 'discard')
...
print("Before Transform: Total Duration is {}".format(time.time() - t0))
print("Before Transform: Average Duration is {}".format((time.time() - t0) / (len(test_data)*batch_size)))
...
finetune_net = finetune_net.cast("float16")
...
print("After Transform: Average Duration is {}".format((time.time() - t1) / (len(test_data1)*batch_size)))
